...
async function updateChangeLogVersion(changeLogPath, version, options = {}) {
  ...
  const lines = await readChangeLog(changeLogPath, options);
  ...
  if (versionLineIndex < 0) {
    return Promise.reject('There is no previous version\'s header in changelog file.');
  }
  ...
  lines.splice(changeListIndex, 0, `${prefix} ${version} (${moment().format('YYYY-MM-DD')})`, '');
